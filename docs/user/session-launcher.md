...
- 筛选区可按「缺工作目录 / 缺会话源 / 陈旧」过滤列表；**仅在你切换到这些筛选或展开磁盘占用时**才批量探测健康（不全量扫描后自动探测）。
- 有探测结果时，行上可能显示「缺目录·缺源」角标。
...
1. 展开某个 agent 分组。
2. 在项目 header 右侧点击星标按钮，可收藏整个项目目录。
3. 在 session 行点击星标，可收藏单条 session。
4. 排序优先级：**session 收藏 > 项目收藏 > 最近活跃时间**。
5. 两类收藏都保存在本机，重启后仍会保留；已删除的 session 会在下次保存时被清理出收藏列表。
...
成功启动后，页面上方会出现 **最近启动** 快捷条，可一键再启动。右键最近项可预览将要执行的 resume 命令（不写 wrapper、不执行）。
...
删除只作用于当前行对应的 CLI 本地 session 源文件（Codex/Claude 的 `.jsonl`）、Cursor/Grok session 目录或 OpenCode 数据库中的 session 行，不会删除项目工作目录。删除失败时，列表不会假装移除该 session。
...
“项目服务”默认只包含当前用户、本地监听、非系统/应用目录的 TCP LISTEN 端口。可在规则区配置：

- **忽略端口**：逗号分隔端口号，匹配项不展示。
- **项目路径前缀**：工作目录命中前缀时，扩大为项目服务。

端口行会显示进程、PID、地址、状态和工作目录；工作目录可复制或打开。loopback 监听可点 **打开** 在浏览器访问 `http://127.0.0.1:<port>`（非 loopback 禁用）。

### 关闭端口服务

1. 在 Port 工具页点击端口行右侧“关闭”，或勾选多行后点“关闭选中”。
2. 多个进程占用同一端口时，可以展开端口后关闭单个进程，或在分组行关闭全部。
3. 在确认弹窗中点击“关闭服务”。

关闭服务会对关闭前重新扫描后仍一致的当前用户 PID 发送 `TERM` 信号，批量关闭仍为 all-or-nothing。前端不会直接传 PID；如果端口记录过期、记录已变化、进程不属于当前用户或系统拒绝关闭，应用会显示错误并保留列表。
