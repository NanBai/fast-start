...
CodeStable 技能默认面对的是有历史、会中断、工具可能失败的真实仓库。下面这些情况是标准路径，不是异常发挥：

### 多 git 仓库

- 启动时先读 `.codestable/attention.md`，如果里面说明"工作区根目录不是 git 仓库 / 子项目各自有 git"，所有 `git status` / `git diff` / `git log` / `git add` / `git commit` 都按那份说明切到子仓库执行。
- 在根目录执行 `git` 报 "not a git repository" 时，不要把它当成无改动或阻塞；回到 attention.md 找子仓库边界，分别检查。
- 最终汇报需要按仓库分组说明改动和验证结果。
- 提交时按仓库分别提交，commit message 需要明确说明是哪个子项目的改动（如 `[backend] 实现多量表编排` / `[frontend] 多行资产选择组件`）。

### 术语 / 禁用词反向 grep

涉及安全边界、产品承诺、权限、隐私、远控、支付、数据一致性、兼容迁移等语义时，design / implement / acceptance 都要把术语当成可验证对象：

- **design 阶段**写出：
  - 禁用词列表：哪些词/短语绝对不能出现在用户可见位置或当前能力描述中。
  - 允许保留位置：内部 code 标识符、REST payload 字段名、数据库表名/字段名、历史迁移脚本常量、兼容性字段。每个保留项需说明保留理由。
  - grep 目录范围：前端源码（src/）、后端源码（按语言确定）、SQL 脚本、测试代码、用户文档（README / docs/ / guides/）。
  - 示例格式：
    ```yaml
    forbidden_terms:
      - term: "机器人远程控制"
        reason: "安全边界：系统不提供远程控制能力"
        allowed_contexts:
          - "internal field: remoteControlEnabled（兼容字段，始终false）"
          - "migration script: 20XX-XX-XX-remove-remote-control.sql"
        grep_dirs: ["ybb-interview/src/", "ybb-interview-web/src/", "*.sql", "docs/"]
    ```
- **implement 阶段**新增术语或实际改动超出 design 术语表时，先回填 design 并 grep 防冲突。
- **acceptance 阶段**执行反向 grep，覆盖 design 定义的所有目录；命中结果按四类处理：
  - **必须修**：用户可见文案、页面标题、按钮 label、默认配置、错误消息、审计日志描述、指南中的当前能力描述、API 文档的能力声明。
  - **可兼容保留**：design 已明确允许的内部 code、REST payload、表名、字段名、source event、迁移匹配常量。报告里必须引用 design 中的保留理由。
  - **历史描述**：design / acceptance / changelog / learning 里用于说明"从旧到新"的对比文字，可保留但必须有明确时态标记（"之前"/"曾经"/"已废弃"）。
  - **工具/测试假阳性**：注释、测试 mock 数据、第三方库名称。记录过滤依据，不静默忽略。
- grep 命令示例：`grep -r "机器人远程控制" ybb-interview/src/ ybb-interview-web/src/ docs/ --exclude-dir=node_modules --exclude-dir=.git`

### 验证证据

- **前端改动验证优先级**：
  1. 用户人工验证：用户已明确"我在浏览器验证过，功能正常"时，作为第一等证据，报告需记录验证时间和验证内容摘要。
  2. 代码审查 + 类型检查：`tsc --noEmit` / `pnpm type-check` 通过 + 相关组件代码审查通过。
  3. 单元测试：相关组件/函数单测通过。
  4. 浏览器自动化截图：仅当以上证据不足时补充（如用户未验证、改动涉及视觉呈现细节）。
  
  用户人工验证 + 代码审查 + 类型检查 已足够作为前端验收证据，不必强制要求浏览器自动化。

- **工具失败分类**：命令参数错误、工具 bug、环境缺失、真实代码失败。只有真实代码失败才直接算验收失败；工具 bug 要记录并用次优证据补足（如 Playwright 启动失败 → 改用人工验证 + 代码审查）。
- **全量检查失败但失败点在未改动范围**：报告要写清文件、错误摘要和为何不顺手修；不要把无关修复混进当前 feature / issue。

### 验收阶段补修边界

验收过程中发现偏差可以当场修复，但需要明确边界：

- **可以补修**：术语/文案不一致、注释缺失或误导、小逻辑修正（边界检查、默认值、错误消息）、测试用例补充、遗漏的类型标注。
- **不能补修**（必须退回 impl 或记后续 issue）：大结构变化（新增模块、改接口签名、重构数据流）、design 未覆盖的新功能点、跨模块影响的修改。
- **补修记录**：验收报告必须有"验收阶段补修"固定小节，列出补修文件、补修原因、补修内容摘要。不能把补修内容散落在各验收项里或完全不记录。
...
**历史状态兼容**：如果接手时历史 checklist 的 `checks` 已经是 `done`（实现阶段遗留状态，旧版本技能或手动标记），acceptance 允许把它视为"待规范化状态"，按以下规则处理：
- 逐项核对该 check 是否真的通过。
- 通过 → 规范化为 `passed`；未通过 → 规范化为 `failed`。
- 验收报告"验收阶段补修"小节记录：接手时状态为 `done`，本次规范化为 `passed` / `failed`，并列出各项最终状态。
- 不能因为状态不是 `pending` 就跳过核对或直接标 `passed`。

**steps 状态不改**：`steps[].status` 在 implement 阶段已标记为 `done`，acceptance 不再修改它；如果 steps 仍有 `pending` 说明实现未完成，应退回 impl 而不是在 acceptance 里继续实现。
...
active/completed → superseded / abandoned（cs-roadmap update 模式，规划整体被新方向取代或用户明确放弃时改主文档 status）
```

`done` / `dropped` 是终态。需要回退重做的新加一条 slug 略改的条目，不改终态。

roadmap 主文档 `status` 建议值：

- `active`：仍是后续 feature 的有效规划输入。
- `completed`：条目已完成或终止，保留作历史。
- `superseded`：被新方向 / 新 roadmap / 已落地架构取代；feature-design 不能再直接按旧契约开工，必须先让用户确认是否走 update。
- `abandoned`：用户明确放弃，不再作为后续输入。
...
1. `cs-learn`：沉淀经验
2. `cs-decide`：长期约束 / 选型
3. `cs-guide`：开发者 / 用户指南。触发条件：用户可见流程变化、开发者接入步骤变化、运行/配置方式变化；仅内部 label / 注释变化通常不用。
4. `cs-libdoc`：公开 API 参考。触发条件：公开组件、函数、命令、接口契约新增或变更；纯内部实现或文案变化通常不用。
5. `scoped-commit`
