...
**项目简介**：一个 Tauri 桌面应用，聚合展示 codex / claude-code / cursor / grok-build / opencode / oh-my-pi 六个 AI CLI agent 的最近 session，选中后一键拉起外部终端、cd 到工作目录、以 session ID resume 对应 agent，快速恢复工作上下文。另提供本机开发端口监控（Port 工具页，见 `port_monitor`），以及 **Providers 工具页**（多 CLI 上游/模型切换：Grok 登录方式见 `grok_provider`——API 供应商档案 / 官方账号切换 / 用户触发拉模型与 config 预览 / 隐私保护 / 卡片排序置顶；Oh My Pi 窄支持见 `omp_provider`——`models.yml` 列表与 `config.yml` `modelRoles` 切换）。
...
- **快速访问**：搜索、最近天数、项目收藏与 session 收藏都在前端从扫描结果派生；稳定顺序是“最近天数过滤 → 搜索过滤 → session 收藏 > 项目收藏 > 时间 → agent/project 渲染”。可按 Agent 或按项目（跨 CLI）视图切换。搜索不下推到 scanner，不触发重新扫描。
- **恢复**：用户点"启动"→ `preflight_session`（cwd/program/源）→ 有 block 则失败不写 wrapper → 否则 `CommandSpec` + `TerminalLauncher` 开窗 resume；**仅成功**后写入 `recent_launches`（上限 20）。`preflight_launch` / `preview_launch_command` 只读。
- **卫生**：`inspect_session_health` 复用 `session_source::check_session_source`（OpenCode=行）；列表可筛陈旧并展示磁盘占用聚合（前端对 approxBytes 求和）。
- **删除**：单条右键或批量勾选 → 确认 → `delete_session` / `delete_sessions` 走同一全路径（含 OpenCode 行删）；bulk partial success + failures；前端不传路径。
- **Port**：`port_ignore_ports` 隐藏；`port_protect_ports` 使 terminate 命中保护端口时**整批失败**；UI 可按 workingDirectory 分组并一键关项目端口（未知目录无危险一键）。
- **Grok 出站**：`grok_fetch_models` / `grok_test_connection` 仅用户触发；`grok_config_health` 只读消毒诊断 + issues[]（无 apiKey/绝对路径）；`grok_preview_apply` 不写盘。
...
| `CliType` | 枚举：codex / claude-code / cursor / grok-build / opencode / oh-my-pi | `src-tauri/src/models.rs` |
| `TerminalType` | 枚举：System / ITerm2 / Ghostty / WezTerm | `src-tauri/src/models.rs` |
...
| `TerminalLauncher` trait | `terminal_type()` / `is_available()` / `supports_tab()` / `launch(&CommandSpec, LaunchMode)`；每终端一实现 | `src-tauri/src/launcher/` |
| `check_session_source` | preflight 与 health 共享源探测；OpenCode=行存在 | `src-tauri/src/session_source.rs` |
| `PreflightResult` | 启动预检 ok/checks/preview | `src-tauri/src/launch_preflight.rs` |
...
| `favorite_project_dirs` / `favorite_session_ids` | 本机偏好收藏；sanitize 对齐已扫描 sessions；只影响前端排序 | `src-tauri/src/preferences.rs` |
| `recent_launches` | 成功 launch 后的最近记录（上限 20） | `src-tauri/src/models.rs` / preferences |
| `AppState` | sessions 缓存 + port 规则 + recent_launches + scan-cache / ops_ready + 终端/主题偏好 | `src-tauri/src/state/` |
...
├── omp_provider.rs Oh My Pi models.yml / config.yml 角色模型窄支持（list/health/set）
...
src/               React 前端：App 负责 Session/Port/Grok 三工具页与键盘编排；
                   hooks/ 承载 usePreferences / useSessions / usePorts / useGrokProviders / useOmpProviders；
...
| oh-my-pi | `~/.omp/agent/sessions/**/*.jsonl`（`OMP_HOME` / `PI_CODING_AGENT_DIR` 可覆盖） | JSONL 中 `type=session` 的 `cwd`（文件内可先出现 title 行） | `omp -r <id>`（或 `--resume`） |

六家都是 `cd <cwd> && continue <id>` 模式（cursor 的 cd 必须准确，否则 resume 失败）。cursor 不用 `cursor-agent ls`（要 TTY）。

删除目标和扫描来源一致：codex / claude-code / oh-my-pi 删除扫描命中的 `.jsonl` 文件；cursor 删除 chat 目录；grok-build 删除 session 目录；opencode 删除 `session` 表对应行（不删 db 文件）。删除动作不按 CLI 原始 `session_id` 批量匹配，只处理 UI 当前行对应的 `Session.id`。
...
- `program` 限白名单：`codex` / `claude` / `cursor-agent` / `grok` / `opencode` / `omp`
...
> 当前边界：六家 CLI 都是 `cd <cwd> && continue <id>` 语义；安全校验在 `CommandSpec` 层统一完成，终端实现只负责选择窗口 / tab 并启动 wrapper。
...
- Codex / Claude Code / Oh My Pi 只允许删除文件；Cursor / Grok Build 只允许删除目录；OpenCode 删除 SQLite session 行
- 删除失败必须显式报错，不能从列表假移除；删除成功只移除当前缓存 id，不按 `session_id` 清理其他项
- 删除能力不得删除 `project_dir` 指向的工作目录、CLI 全局目录、账号配置、缓存目录或其他 session

## 变更日志

### 2026-07-13 - Oh My Pi (omp)

- 新增 `CliType::OhMyPi`：扫描 `~/.omp/agent/sessions/**/*.jsonl`（兼容 title 先行）、`omp -r <id>` resume、File 删除、security 白名单 `omp`。
- Providers 窄支持：`omp_list_providers` / `omp_get_config_health` / `omp_set_role_model`（读 models.yml、受控写 config.yml modelRoles + 备份）。

### 2026-07-13 - ux-perf polish

- 前端：`SessionWorkspace` 与 App 壳拆分；session 行轻量虚拟列表；inspect **按需**（陈旧筛选/磁盘面板）。
- 后端：`state/session_ops.rs` 从 `state/mod.rs` 抽出 launch/delete/preflight/health/prefs。
- 启动 UI 不再双 preflight；后端 `launch_session` 门闩保留。

### 2026-07-13 - power-extend

- 启动预检门闩 + 共享 `session_source`；健康探测与磁盘占用聚合；摘要 `clean_summary` ≤160。
- 批量删除 `delete_sessions`（≤50，partial success）；port `port_protect_ports`；Port UI 按项目目录分组。
- `grok_config_health` issues 诊断；WezTerm 适配器；`cli_contract` 注册完整性测试；`docs/dev/cli-extension-checklist.md`。
