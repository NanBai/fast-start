...
```bash
python3 <cs-onboard skill 目录>/tools/codestable-goal-consistency-gate.py --roadmap .codestable/roadmap/session-launcher-next-wave
```
...
写 `.codestable/roadmap/session-launcher-next-wave/goal-audit.md`：
...
# session-launcher-next-wave Goal 最终审计
...
同时写 `.codestable/roadmap/session-launcher-next-wave/goal-evidence-summary.md` 或在 goal-audit 第 7 节等价内嵌：
