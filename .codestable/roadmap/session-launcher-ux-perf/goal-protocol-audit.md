...
```bash
python3 <cs-onboard skill 目录>/tools/codestable-goal-consistency-gate.py --roadmap .codestable/roadmap/session-launcher-ux-perf
```
...
写 `.codestable/roadmap/session-launcher-ux-perf/goal-audit.md`：
...
# session-launcher-ux-perf Goal 最终审计
...
同时写 `.codestable/roadmap/session-launcher-ux-perf/goal-evidence-summary.md` 或在 goal-audit 第 7 节等价内嵌：
