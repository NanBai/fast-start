...
- **聚合五类 CLI**：Codex（`~/.codex`）、Claude Code（`~/.claude`）、Cursor Agent（`~/.cursor`）、Grok Build（`~/.grok`）、OpenCode（`~/.local/share/opencode`）
- **按项目整理**：同一仓库下的多条 session 归在同一项目标题下
- **搜索与筛选**：按项目名、路径、session 摘要、agent 名搜索；可按最近 1～30 天或全部时间范围查看
- **收藏置顶**：按**项目目录**收藏，常用仓库排在前面
- **一键恢复**：在 Terminal.app、iTerm2、Ghostty 或 WezTerm（若已装）中**新窗口 / 新标签**启动；启动前只读预检 cwd / PATH / 源
- **本地删除**：右键单条或勾选批量删除（≤50，partial success）；不删项目代码目录
- **陈旧筛选与磁盘占用**：按需探测缺目录/缺源；可折叠查看源载体近似体积
- **端口监控**：查看本机 TCP/UDP 端口；保护端口列表拦截 terminate；可按项目目录分组关闭
- **关闭服务**：对选中的当前用户端口进程发送 `TERM` 信号，关闭后自动重新扫描
- **Grok 供应商**：Profile 切换、健康诊断 issues、模型拉取与 config 预览（用户触发）
...
依赖：macOS、Node.js、pnpm、Rust（Cargo）、Tauri 2 工具链；本机至少一种上述 CLI 的历史 session；至少一个可用终端（Terminal.app / iTerm2 / Ghostty / WezTerm）。
...
1. **扫描**：Rust 后端读取各 CLI 在本机的 session 存储（如 Codex/Claude 的 jsonl、Cursor 的元数据与 `store.db`、Grok Build 的 `summary.json`、OpenCode 的 `opencode.db`），解析出 session id、项目路径、摘要、时间等。
...
4. **启动**：根据你选择的终端类型，通过安全校验后的 wrapper / AppleScript 等方式在外部终端执行允许的 resume 程序（`codex`、`claude`、`cursor-agent`、`grok`、`opencode`），并切换到对应 `cwd`。
